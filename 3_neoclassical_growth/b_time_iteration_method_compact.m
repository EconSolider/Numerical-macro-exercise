...
% the min and max value of k_grid should contain steady state of k
min_k=0.05;
max_k=40;
num_k=40;
...
tol  = 1.0e-5;  % 收敛容忍度
options = optimoptions('fsolve','Display','none','Algorithm', 'levenberg-marquardt', 'MaxFunctionEvaluations', 1000);
...
while (diff > tol && it < maxiter)
    ...
    residual=@(c_fcn) b_Euler_residual(c_fcn,k_grid,st);
    c_fcn_1=fsolve(residual,c_fcn_0,options);
